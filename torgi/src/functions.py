...
def loading_railways(railway_source: str) -> pd.DataFrame:
    esr_df = pd.read_csv(os.path.join('data', 'const_filters', 'railways', 'esr.csv'), sep=';')
    osm2esr_df = pd.read_csv(os.path.join('data', 'const_filters', 'railways', 'osm2esr.csv'), sep=';')
...
def generate_map(lots_df, raylway_df, map_path):
    try:
    ...
    try:
        ...
        # category_layers = []
            ...
            logger.info('На карту добавлены ЖД станции')
    except Exception as e:
        logger.error(f'Ошибка в добавлении ЖД станций: {e}')
    ...
    try:
        ...
        for cat in categories:
            ...
            for _, r in tmp_df.iterrows():
                ...
                rent_period = (r['rent_period'] if not pd.isnull(r['rent_period']) else 'Не указано') if 'rent_period' in tmp_df.columns else 'Не указано'
                price = (f"{r['priceMin']} руб." if not pd.isnull(r['priceMin']) else 'Не указано') if 'priceMin' in tmp_df.columns else 'Не указано'
                price_step = (f"{r['price_step']} руб." if not pd.isnull(r['price_step']) else 'Не указано') if 'price_step' in tmp_df.columns else 'Не указано'
                deposit_price = (f"{r['deposit_price']} руб." if not pd.isnull(r['deposit_price']) else 'Не указано') if 'deposit_price' in tmp_df.columns else 'Не указано'
                area = (f"{r['area']} м²" if not pd.isnull(r['area']) else 'Не указано') if 'area' in tmp_df.columns else 'Не указано'
                bidd_start_date = (f"{r['bidd_start_date'].strftime('%d.%m.%Y %H:%M')} ({r['timeZoneName']})" \
                    if not pd.isnull(r['bidd_start_date']) else 'Не указано') if 'bidd_start_date' in tmp_df.columns else 'Не указано'
                bidd_end_time = (f"{r['biddEndTime'].strftime('%d.%m.%Y %H:%M')} ({r['timeZoneName']})" \
                    if not pd.isnull(r['biddEndTime']) else 'Не указано') if 'biddEndTime' in tmp_df.columns else 'Не указано'
                auction_start_date = (f"{r['auction_start_date'].strftime('%d.%m.%Y %H:%M')} ({r['timeZoneName']})" \
                    if not pd.isnull(r['auction_start_date']) else 'Не указано') if 'auction_start_date' in tmp_df.columns else 'Не указано'
                auction_link = (f"<br><a href={r['auction_link']} target='_blank'>Аукцион</a><br>" \
                    if not pd.isnull(r['auction_link']) else "<br>") if 'auction_link' in tmp_df.columns else "<br>"
                cad_link = (f"<a href={r['cad_link']} target='_blank'>Кадастровая карта</a><br><br>" \
                    if not pd.isnull(r['cad_link']) else "<br><br>") if 'cad_link' in tmp_df.columns else "<br><br>"
        ...
        logger.info('Земельные участки успешно добавлены на карту.')
    except Exception as e:
        logger.error(f'Ошибка при добавлении на карту земельных участков: {e}')

    try:
        map_fullname = os.path.join(map_path, 'map.html')
        current_map.save(map_fullname)
        logger.info(f'Карта сохранена по адресу: {map_fullname}')
    except Exception as e:
