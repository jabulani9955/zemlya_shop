...
def data_processing(
    ...
    for subject in subjects: 
        ...
        df['rosreestr_info'] = df['cadastral_number'].apply(lambda x: get_coords_from_cadastral_number(x))
    ...
    try:
        for filename in sorted(os.listdir(results_path))[:-2]:
            os.remove(os.path.join(results_path, filename))
    except Exception as e:
        logger.critical(f'Ошибка при удалении старых файлов.\n{e}', exc_info=True)

    try:
        fullname_file = os.path.join(results_path, f'TORGI_{time_now}.csv')
        full_df.to_csv(fullname_file, index=False)
        logger.info('Файл успешно сохранён.')
    except Exception as e:
        logger.critical(f'Ошибка при сохранении файла.\n{e}', exc_info=True)
    
    logger.info('Создаю карту...')
    try:
        generate_map(lots_df=full_df.copy(), raylway_df=full_railway_stations.copy(), map_path=os.path.join('data', 'maps'))
        logger.info('Карта создана.')
    except Exception as e:
        logger.critical(f'Ошибка в генерации карты: {e}', exc_info=True)
